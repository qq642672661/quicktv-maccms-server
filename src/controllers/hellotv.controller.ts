...
class HelloTVController {
  async getTabList(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
  ...
  async getTabContent(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      logger.info(`📋 [HelloTV] 获取Tab内容: tabId=${tabId}, page=${page}`);
      
      const content = await hellotvService.getTabContent(
        tabId,
  ...
  async getLiveChannelGroups(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
